...
const LevelCard = ({
  ...
  const getDescription = () => {
    if (!nivel.description || nivel.description === `Nivel ${nivel.name}` || nivel.description.length < 20) {
  ...
  const handleToggleChange = () => {
    // Si el nivel no está completado y se intenta activar (isActive es false antes del cambio)
    if (!nivel.completed && !isActive) {
      onAttemptActivateUncompleted(nivel.name) // Llamar a la función para mostrar el modal
      return // Detener el cambio de estado
    }
    onToggle(nivel.id) // Proceder con el cambio de estado normal
  }

  return (
    <div className={`p-6 rounded-xl border-2 transition-all duration-200 hover:shadow-md ${getCardStyle()}`}>
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-gray-900 text-base mb-1">{nivel.name}</h3>
            <p className="text-sm text-gray-600 leading-relaxed mb-2">{getDescription()}</p>
            {nivel.duration && <p className="text-xs text-gray-500">Duración: {nivel.duration} horas</p>}
          </div>
        </div>
        ...
        <ProtectedAction module="Asignación de Niveles" privilege="assign">
          <label className="relative inline-flex items-center cursor-pointer group">
            <input
              type="checkbox"
              checked={isActive}
              onChange={handleToggleChange} // Usar la nueva función de manejo
              disabled={disabled}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-sm">
          ...
          {isActive && !disabled && (
            <div className="flex items-center text-green-700">
              ...
              <span className="font-medium">{hasChanged ? "Se activará al guardar" : "Nivel activo"}</span>
            </div>
          )}
        ...
        <span
          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
            nivel.completed ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
          }`}
        >
